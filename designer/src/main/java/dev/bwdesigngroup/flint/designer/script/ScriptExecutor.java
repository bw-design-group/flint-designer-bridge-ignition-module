package dev.bwdesigngroup.flint.designer.script;
...
/**
 * Executes Python scripts in the Designer's script context. Captures stdout/stderr output and
 * handles timeouts. Supports session persistence for REPL-style interaction.
 */
public class ScriptExecutor {
    private static final Logger logger = LoggerFactory.getLogger("flint.Designer.Script");
    ...
    public ScriptExecutor(DesignerContext context) {
        this.context = context;
        this.executor =
                Executors.newCachedThreadPool(
                        r -> {
                            Thread t = new Thread(r, "FlintScriptExecutor");
                            t.setDaemon(true);
                            return t;
                        });
    }
    ...
    public ExecuteScriptResult execute(
        ...
        try {
            ...
            logger.debug(
                    "Script executed successfully in {}ms (session={})", executionTime, sessionId);
            ...
            logger.error("Script execution failed: {}", errorMessage, cause);
    ...
    /**
     * Executes the script using the Designer's ScriptManager. If sessionId is provided,
    ...
    private void executeScript(
        ...
        try {
            ...
            if (sessionId != null) {
                locals =
                        sessionLocals.computeIfAbsent(
                                sessionId,
                                k -> {
                                    logger.debug("Creating new session locals for: {}", k);
            ...
            // Execute the code
            scriptManager.runCode(code, locals, "<flint-console>");
     ...
     * that script library changes in the Designer are reflected immediately. User-defined variables
     * in session locals are preserved.
     */
