...
class FlintDebugger(bdb.Bdb):
    ...
    def _handle_stop(self, frame, reason):
        ...
        for sf in stack_frames:
            from dev.bwdesigngroup.flint.common.protocol.methods.debug import DebugStackTraceResult
            java_frame = DebugStackTraceResult.StackFrame(
                sf['id'],
                sf['name'],
                sf['filePath'],
                sf['line']
            ...
            if 'modulePath' in sf:
                java_frame.setModulePath(sf['modulePath'])
            frame_list.add(java_frame)

        self.session.setCurrentStackFrames(frame_list)
    ...
    def _wait_for_command(self):
        ...
        from dev.bwdesigngroup.flint.designer.debug import DebugCommand
        from dev.bwdesigngroup.flint.common import FlintConstants
        ...
        while self._paused and not self._terminate:
            ...
            if cmd_type == DebugCommand.Type.CONTINUE:
                self._paused = False
            ...
            elif cmd_type == DebugCommand.Type.STEP_OVER:
                self._paused = False
            ...
            elif cmd_type == DebugCommand.Type.STEP_INTO:
                self._paused = False
                self._continue_mode = False  # Stop at next line
                self.set_step()
                self.session.clearReferences()
            ...
            elif cmd_type == DebugCommand.Type.STEP_OUT:
                self._paused = False
            ...
            elif cmd_type == DebugCommand.Type.PAUSE:
                # Already paused
                pass

            elif cmd_type == DebugCommand.Type.TERMINATE:
    ...
    def _register_frame_references(self, frame):
        ...
        while f is not None:
            ...
            from dev.bwdesigngroup.flint.designer.debug import DebugSession
            frame_info = DebugSession.FrameInfo(f.f_locals, f.f_globals)
            self.session.registerFrameReference(frame_id, frame_info)
